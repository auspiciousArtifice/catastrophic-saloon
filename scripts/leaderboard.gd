...
func _on_authentication_request_completed(result, response_code, headers, body):
	...
	file.store_string(json_parse.result.player_identifier)
	file.close()
	
	# Save session_token to memory
	session_token = json_parse.result.session_token
	
	# Print server response
	print(json_parse.result)
...
func _on_leaderboard_request_completed(result, response_code, headers, body):
	...
	for n in json_parse.result.items.size():
		leaderboardFormatted += str(json_parse.result.items[n].rank)+str(". ")
		leaderboardFormatted += str(json_parse.result.items[n].player.id)+str(" - ")
		leaderboardFormatted += str(json_parse.result.items[n].score)+str("\n")
